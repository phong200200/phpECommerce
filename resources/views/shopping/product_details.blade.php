	...
	<div class="span9">
		...
		<div class="well well-small">
			<div class="row-fluid">
				...
				<div class="span7">
					<h3>{{{$product -> name}}} - <strong>[{{number_format($product -> price)}} đ]</strong></h3>
					<hr class="soft" />

					<form class="form-horizontal qtyFrm" method="POST">
						<div class="control-group">
							<input id="theprice" type="hidden" value="{{$product -> price}}">
							<label class="control-label"><span id="strprice">{{number_format($product -> price)}}</span></label>
							<div class="controls">
								<input type="number" id="qty" onchange="ChangeQty(this)" class="span6" placeholder="Qty">
							</div>
						</div>
						<script>
							function ChangeQty(this){
								$('strprice').setvalue(number_format($('#theprice').getvalue() * $('#qty').getvalue()))
							}
							ChangeQty()
						...
						<p>
							<button type="submit" onclick="Send('/shopping/cart/addpostcart/')" class="shopBtn"><span class=" icon-shopping-cart"></span> Add to cart</button>
					</form>
				</div>
			</div>
			<hr class="softn clr" />
